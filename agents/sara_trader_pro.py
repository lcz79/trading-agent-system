...
class SaraTrader:
    ...
    # ... (il resto del codice che hai fornito) ...
    # (Ho omesso il resto per brevità, ma incolla l'intero file che mi hai dato)
    ...
    def execute_order(self, signal: dict) -> dict:
        try:
            ...
            if not symbol or side not in ("buy", "sell", "long", "short"):
                return {"status": "error", "message": "Segnale invalido: side non riconosciuto."}
    ...
    def _list_open_symbols(self) -> List[str]:
        try:
            ...
            return list(set([p['symbol'] for p in pos if p.get('symbol') and float(p.get('contracts', 0) or p.get('info', {}).get('size', 0)) > 0]))
        except Exception as e:
