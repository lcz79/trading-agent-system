...
class WebbyExecutor:
    def __init__(self, agents_cfg_path="config/agents_config.json", risk_cfg_path="config/risk_parameters.json", ex_cfg_path="config/exchanges.json"):
        ...
        # La modalità LIVE è ora controllata da una variabile d'ambiente.
        # Questo ci permette di attivarla solo sul server di produzione.
    ...
    def plan_new_proposals(self):
        ...
        for symbol, decision_pack in sara.items():
            ...
            if bias not in ("LONG", "SHORT") or not self._should_fire(symbol): continue
            mark_signal = decision.get("src", {}).get("mark", {})
            if mark_signal.get("signal") not in ("LONG", "SHORT"): continue
            ex_name = "bybit"
            ...
            if amount_precision is not None: qty_est = round(qty_est, int(amount_precision))
            if qty_est <= specs.get('min_qty', 0): continue
            proposal = { "symbol": symbol, "side": bias, "entry": entry, "sl": sl, "tp": tp, "qty_est": qty_est, "score": decision.get("score", 0), "logic": mark_signal.get("logic", "N/A") }
