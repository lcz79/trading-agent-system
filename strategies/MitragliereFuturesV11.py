"""
MitragliereFutures v11 - Calibrated v9

v9 era positivo (+0.41%) con 58.6% WR ma solo 29 trade in 3 anni.
v10 ha peggiorato allargando troppo (lookback 4, no green_candle).

v11 = calibrazione fine di v9:
- LONG: lookback BB 3 candele (era 2), rsi_confirm=42 (era 38)
  → manteniamo green_candle + rsi_rising (filtri di qualità)
  → atteso: 30-50 long trade (2x v9) con WR simile ~55-65%
- SHORT: identico a v9 (funzionava bene: 6 ROI 100%, 4 trailing 69%)
  → in_macro_bear + in_downtrend_4h + 2-candle RSI peak
- STOP: identico v9 (pHSL=-0.05, hard=-0.08)
"""
import numpy as np
import pandas as pd
...
class MitragliereFuturesV11(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # === SHORT: in MACRO BEAR + 4h downtrend + 2-candle RSI peak ===
        rsi_peaked = (
        ...
        # Lookback: BB_lower toccato nelle ultime 3 candele (era 2 in v9)
        bb_was_touched = (
