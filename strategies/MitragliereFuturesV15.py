"""
MitragliereFutures v15 - Natural Mean Reversion Target

INSIGHT CRUCIALE: v9 ha entrate ottime (BB dip + reversal confermato) ma exit sbagliata.
Problema: minimal_roi=3% taglia i trade a +3% quando il target NATURALE del mean-reversion
è il ritorno a sma50_4h, che è spesso +5-15% dall'entry (BB_lower è molto sotto sma50_4h).

ESEMPIO:
- Prezzo: 100 (entry, al BB_lower)
- SMA50_4h: 108 (8% sopra entry)
- Exit v9: chiude a 103 (+3%)
- Exit v15: chiude a 108 (+8% = 16% su 2x leveraged)

R:R v9: +3% target vs -5% stop = 0.6:1 (negativo!)
R:R v15: +8% target vs -5% stop = 1.6:1 (positivo!)

ENTRY: identico v9 (migliore di tutte):
...
EXIT naturale (invece di minimal_roi fisso):
- LONG exit: close > sma50_4h * (1 + exit_margin) = prezzo tornato in uptrend
  Oppure: close < bb_lower * 0.99 (reversal fallito = exit rapido)
- SHORT exit: close < sma50_4h * (1 - exit_margin) = prezzo tornato in downtrend
  Oppure: close > rsi_short_peak (RSI recovered)

STOP: pHSL=-0.05, hard=-0.08 (identico v9)
NO minimal_roi fisso (lascia il trade correre fino al target naturale)
"""
import numpy as np
import pandas as pd
...
class MitragliereFuturesV15(IStrategy):
    INTERFACE_VERSION = 3

    timeframe = '1h'
    can_short = True
    minimal_roi = {"0": 100}   # NO ROI exit fisso: exit tramite exit_signal
    stoploss = -0.08
    ...
    # === LONG: BB lower extreme + reversal confirmation (v9) ===
    ...
    # Exit margin: quanto sopra sma50_4h per considerare il reversal completo
    ...
    # BB fail exit: quanto sotto BB_lower = reversal fallito
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # SHORT: in_macro_bear + in_downtrend_4h + 2-candle RSI peak (v9)
        rsi_peaked = (
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Exit quando il dip è finito e siamo tornati in uptrend
        dataframe.loc[
            dataframe['close'] > sma50_4h * (1 + self.exit_margin.value),
            'exit_long'
        ] = 1

        # LONG fail: prezzo scende ulteriormente sotto BB_lower = reversal fallito
        dataframe.loc[
            dataframe['close'] < dataframe['bb_lower'] * self.bb_fail_factor,
            'exit_long'
        ] = 1

        # EXIT SHORT: prezzo torna sotto SMA50_4h = mean reversion completata (short)
        # Oppure RSI recovered sopra soglia (falso picco)
        dataframe.loc[
            (dataframe['close'] < sma50_4h * (1 - self.exit_margin.value)) |
