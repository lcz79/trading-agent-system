"""
MitragliereFutures v9 - Confirmed Reversals

PROBLEMA v8: long_bull_dip entra sul BB_lower mentre il prezzo sta ancora scendendo.
- 26/57 long = hard stop immediato (price continua a scendere dopo BB_lower touch)
- pHSL=-0.03 troppo stretto: qualsiasi 1.5% move in 1h cripto triggera lo stop
- WR long: 20.7% (trailing) → entra, scende, esce in loss

FIX v9:
1. LONG reversal confirmation: 
   - RSI già in risalita (rsi > rsi.shift(1)) = momentum sta girando 
   - Candela verde (close > open) = selling pressure sta finendo
   - Questo RITARDA l'entry ma migliora drasticamente il WR
   
2. Stop più larghi per sopravvivere ai normali wiggle 1h:
   - pHSL default = -0.05 (2.5% price move con 2x leva)
   - Hard stoploss = -0.08 (4% price move = hard floor assoluto)
   - R:R: 3% profit vs 5% max loss → serve ~63% WR per breakeven
   
3. SHORT: manteniamo la 2-candle RSI peak confirmation (funziona bene: 66.7% WR)

LOGICA ATTESA:
- In bull market: compriamo DOPO che il rimbalzo è confermato, non alla prima
  caduta al BB_lower. Meno trade ma molto più alti WR.
- In bear market: short peak confirmation già funziona bene.
"""
import numpy as np
import pandas as pd
...
class MitragliereFuturesV9(IStrategy):
    INTERFACE_VERSION = 3

    timeframe = '1h'
    can_short = True
    minimal_roi = {"0": 0.03}
    stoploss = -0.08               # Hard floor: 4% price con 2x (più largo per sopravvivere ai wiggle)
    ...
    # STOPLOSS più largo per sopravvivere ai normali wiggle 1h crypto
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # LONG: prezzo sotto sma50_4h (dip in corso in bull market)
        ...
        # === SHORT: 2-candle RSI peak confirmation in MACRO BEAR downtrend ===
        rsi_peaked = (
        ...
        # === LONG: BB dip in MACRO BULL con REVERSAL CONFIRMATION ===
        # KEY FIX: aspettiamo che il reversal sia già iniziato:
        # 1. Prezzo ha TOCCATO il BB_lower nella candela precedente (o questa)
        # 2. RSI già in risalita: rsi > rsi.shift(1) (momentum sta girando)
        # 3. Candela verde: close > open (selling pressure finisce)
        # Questo significa: prezzo toccò il fondo, ORA sta rimbalzando → entry più sicura
        bb_was_touched = (
        ...
        enter_long = (
            ...
            rsi_rising &                # RSI già in risalita (reversal iniziato)
            green_candle &              # Candela verde (momentum girando)
            low_volatility &
            vol_ok
        )
