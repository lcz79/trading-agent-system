"""
MitragliereFutures v7 - Filtered Mean Reversion

FIX CHIAVE basati su analisi 3 anni (2023-2026):

PROBLEMA: 41 hard stop su 134 trade = -$52.44 (senza questi → +$13.59 profit)

FIX 1: LONG filter più forte (daily SMA200)
  - Prima (v6b): close > sma50_4h → compra dead-cat bounce in bear macro
  - Ora (v7): close > sma50_4h AND close > sma200_1d → solo veri uptrend

FIX 2: ATR filter (no entry in alta volatilità)
  - Se ATR > 1.5% del prezzo → mercato troppo volatile per i nostri stop → skip
  - Elimina le entrate nei mesi 0% WR (May 2025, Sep 2025, Oct 2025)

FIX 3: Stop più stretti (R:R migliore)
  - pHSL = -0.03 (1.5% price con 2x leva, era -0.05)
  - Hard stop = -0.04 (2% price, era -0.07)
  - ROI = 3% target → R:R = 3:3 = 1:1 → breakeven a 50% WR
...
MANTENUTO:
  ...
  - Funding rate filter
  - Leverage fix in stoploss_from_open
"""
import numpy as np
import pandas as pd
...
class MitragliereFuturesV7(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_indicators_1d(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Daily SMA200 per macro trend filter sui LONG."""
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Macro trend 1d (SMA200 daily — filtra dead-cat bounce)
        above_sma200_daily = dataframe['close'] > dataframe['sma200_1d']
        above_sma50_daily = dataframe['close'] > dataframe['sma50_1d']
        ...
        # === SHORT: 2-candle RSI peak confirmation in downtrend ===
        rsi_peaked = (
        ...
        # === LONG: BB lower extreme in VERO uptrend (4h + daily SMA200) ===
        # Richiede entrambi: 4h uptrend + sopra SMA200 daily
        # Questo elimina i dead-cat bounce in bear macro
        enter_long = (
            in_uptrend_4h &
            above_sma200_daily &       # FIX principale: macro uptrend confermato
