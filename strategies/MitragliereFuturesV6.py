"""
MitragliereFutures v6 - RSI Peak Confirmation + BB Extreme

APPROCCIO MISTO:
- LONG: prezzo tocca BB_lower in uptrend (bb_extreme funzionava, 70% WR)
- SHORT: RSI cross-below soglia in downtrend (entriamo DOPO il picco del rimbalzo)
  - Attende che RSI fosse > soglia e ora scende sotto (picco confermato)
  - Candela bearish (close < close.shift) = momentum inizia a girare
  - Timing molto migliore vs entrare al picco

MANTENUTO:
- Correlation filter (max 2 same dir)
- Time filter (no 02-06 UTC)
- Funding rate filter
- Leverage fix, SROC bailout fix
"""
import numpy as np
import pandas as pd
...
class MitragliereFuturesV6(IStrategy):
    INTERFACE_VERSION = 3

    timeframe = '1h'
    can_short = True
    minimal_roi = {"0": 0.03}
    stoploss = -0.07  # Hard floor: -3.5% price con 2x leva
    ...
    # === SHORT: RSI cross-below (peak confirmation) ===
    rsi_short_peak = IntParameter(55, 70, default=62, space='buy', optimize=True)

    # === LONG: BB lower extreme ===
    ...
    # STOPLOSS
    pHSL = DecimalParameter(-0.10, -0.03, default=-0.05, decimals=3, space='sell', optimize=True)
    pPF_1 = DecimalParameter(0.008, 0.030, default=0.015, decimals=3, space='sell', optimize=True)
    pSL_1 = DecimalParameter(0.003, 0.015, default=0.010, decimals=3, space='sell', optimize=True)
    pPF_2 = DecimalParameter(0.025, 0.070, default=0.035, decimals=3, space='sell', optimize=True)
    pSL_2 = DecimalParameter(0.015, 0.055, default=0.028, decimals=3, space='sell', optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        in_downtrend = dataframe['close'] < dataframe['sma50_4h'] * (1 - self.trend_margin.value)
        in_uptrend = dataframe['close'] > dataframe['sma50_4h'] * (1 + self.trend_margin.value)

        vol_ok = dataframe['volume'] > dataframe['vol_ma'] * self.vol_mult.value
        no_neg_funding = dataframe['funding_rate'] >= -0.001

        # === SHORT: RSI cross-below (rimbalzo APPENA PICCHATO, ora scende) ===
        # RSI era > soglia candela precedente, ora < soglia = momentum inizia a girare bearish
        # Candela bearish = close < close.shift(1) (prezzo stà già scendendo)
        # 2-candle confirmation: RSI crossed below soglia UNA candela fa, ORA ancora in calo
        rsi_peaked = (
        ...
        # === LONG: BB lower extreme in uptrend ===
        # Prezzo tocca BB lower, RSI oversold → mean reversion verso bb_mid
        enter_long = (
            in_uptrend &
        ...
        dataframe.loc[enter_long, 'enter_tag'] = 'long_bb_dip'
        dataframe.loc[enter_short, 'enter_tag'] = 'short_rsi_peak'
