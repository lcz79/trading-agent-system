        ...
        async function updateNews() {
            try {
                ...
                if(n.news && n.news.length > 0) {
                    document.getElementById('news-box').innerHTML = n.news.map(x=>`
                        <div class="news-item">
                            <div class="news-date">${x.time}</div>
                            <div style="font-size:0.85em; color:#ddd">${x.desc}</div>
                        </div>
                    `).join('');
                } else {
                    document.getElementById('news-box').innerHTML = '<div style="padding:10px; color:#666">No recent news found.</div>';
                }
            } catch(e) {}
        }
